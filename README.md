...
见顶部预览图，测试环境：`SiYuan V3.8.4-alpha.5`、`Windows 11`

- 修复数据库富文本渲染的一些样式问题，已支持渲染代码高亮和公式。
- 支持渲染表格中的富文本内容。
- 补齐【内容块检索面板】中，页签块、页签项的图标。（偏紫色的是页签块，黑色的是页签项）

---
### V2.1.9

测试环境：`SiYuan V3.8.4-alpha.1  `、`SiYuan V3.8.3`、`Windows 11`

见顶部预览图：

- 处理兼容性：当笔记软件版本大于或等于`V3.8.3`时，白板的【通过拖拽方式嵌入文档和内容块】功能恢复可用。
- 白板支持渲染在`SiYuan V3.8.3`中新增的内容块：页签块。
- 白板支持渲染在`SiYuan V3.8.3`中新增的数据库文本字段富文本。

关于白板的【通过拖拽方式嵌入文档和内容块】功能，更新挂件后建议先新建一个空白白板测试一下，不要在已有内容的白板上测试，以免干扰到之前创建的白板。

如果发现没生效，大概是因为受到缓存的影响，白板加载的还是旧的文件，可打开一个白板文档后参考【[禁用缓存后刷新](https://github.com/BryceAndJuly/Whiteboard/issues/100)】进行刷新。

---

### V2.1.8

测试环境：`SiYuan V3.8.3-alpha.3`、`Windows 11`

更新readme文档：

- ❗关于白板的【通过拖拽方式嵌入文档和内容块】功能，当笔记软件版本大于或等于`V3.8.3`时，该功能**不可用，请勿使用。** 因为拖拽的文档或内容块会直接嵌入到白板所在的文档中，而不是白板中，容易引起混乱。

  - 推荐通过本文`四、开始使用`——`6、内容块检索面板`章节所述的检索面板来嵌入内容块。
  - 笔记软件版本小于或等于`V3.8.2`时，该功能未受影响，能正常使用。

经测试，桌面端通过`设置`——`鉴权`——`在浏览器上使用`——`打开浏览器`的方式打开，能正常使用白板的【通过拖拽方式嵌入文档和内容块】功能，但为了稳妥起见，建议还是使用白板的`内容块检索面板`。
