...
- 触发`自动保存`时，鼠标右键弹出的菜单会被关闭，如果觉得这个干扰太大，可以参考下文中的方法默认关闭`自动保存`（临时关闭自动保存的快捷键是`Alt`+`F`）。


> 暂时先更新到这里。待软件新功能经过这段快速迭代完善期、功能基本定型后，再进行适配。

## 一、当前版本

### V2.1.13

参考顶部预览图，主要是修复一些问题以及处理兼容性。测试环境：`SiYuan V3.8.5`、`V3.8.6-alpha.4`、`Windows 11`

- 数据库日历模式：

  - 支持显示周数。（参考：[#19754](https://github.com/siyuan-note/siyuan/issues/19754)）
  - 补充缺失的日历图标。
  - 保留上次打开的视图（周视图/月视图）（参考：[#19767](https://github.com/siyuan-note/siyuan/issues/19767)）。
  - 右上角加上：`上一个（月/周）`   `今天`  `下一个（月/周）`三个按钮，用于切换月份/周。
- 修复问题：当数据库富文本中存在空行时，渲染异常。（笔记软件版本大于或等于V3.8.5时出现）
- 修复问题：页签块设置【左侧页签】后，白板中渲染的仍然是【顶部页签】模式。
- 兼容`SiYuan V3.8.6`版本思维导图。（参考：[#19750](https://github.com/siyuan-note/siyuan/issues/19750)）

---

### V2.1.12

见顶部预览图。测试环境：`SiYuan V3.8.5-beta.5`、`Windows 11`

- 支持渲染列表的导图模式。
- 样式调整：表格内容顶部对齐。（之前是居中对齐）
- 支持渲染数据库的列表视图
- 支持渲染数据库的日历视图（月视图）

  - 卡片进入交互模式后，单击某个字段，可打开该字段的属性面板。
- 修复问题：数据库内容变更后，白板中对应卡片未能自动更新。

关于列表的导图模式，有以下注意事项：

> - 当嵌入的是单个导图时，导图的高度自动铺满卡片，不再固定高度，可通过拖拽卡片边角来修改导图的宽、高
> - 导图左上角有个显示`当前缩放比例`的按钮，卡片进入交互模式后，单击该按钮可重置缩放比例为：100%
> - 卡片右上角本来就有个刷新按钮，刷新后，导图会重置`缩放比例`为适应画布的状态。
> - 保留了导图部分交互，比如：
>
>   - 移动画布：`鼠标左键按下`+`拖动`、`鼠标滚轮`、`Shift`+`鼠标滚轮`
>   - 缩放画布：`Ctrl`+ `鼠标滚轮`
...
见顶部预览图。测试环境：`SiYuan V3.8.4-beta.1`、`Windows 11`
...
> 在较新的版本（比如：`SiYuan V3.7.1`），需要先开启`设置(Alt + P)`——`导出`——`包含关联文档`

## 三、使用前的设置

### 1、添加CSS代码片段

白板创建时，会自动设置所在文档的属性——`别名`的值为：`whiteboard`，这是为了解决打开白板所在文档时页面闪现出文档标题的问题。相应的，你需要在笔记软件的`设置`——`外观`——`代码片段`——`设置`——`CSS`中添加并启用对应的CSS代码片段：
...
需要在笔记软件的`设置`——`外观`——`代码片段`——`设置`——`JS`中添加并启用对应的JS代码片段来辅助获取当前拖拽块的ID。
...
- 安装这个插件是为了
  ...
  - 1、能在白板中悬浮预览笔记内块/文档的内容。
  - 2、在笔记中修改内容块后，白板中对应卡片的内容自动更新。
...
下载完挂件，在新建的空白文档中设置好文档标题，输入`/g`并回车，在挂件列表选择`Whiteboard`即可在当前文档中插入白板，默认铺满文档。
...
```css
工作空间/data/assets/ExcalidrawFiles/20231227015401-w0olmpi.excalidraw
```

使用白板挂件块的ID作为文件名。

- 默认开启`自动保存`，一般在增、删或移动元素后约2s触发保存。如果想默认关闭自动保存或者调整延时时间，可以参考文档顶部描述的方法。
- 如果是临时手动开启/关闭自动保存，可以使用快捷键`Alt+F`，左上角会有提示。
...
> 注意：将笔记中的文档/内容块拖拽到白板中生成卡片后，首次拖拽卡片前需要先单击卡片的`边缘区域`或者白板空白处（这样能让白板重新获得焦点）。
...
白板中嵌入的内容块并不支持直接修改，但是通过悬浮预览窗口等形式修改笔记内容后，白板中对应的内容块能自动更新。
...
>   - 如果只是想临时关闭白板的【内容块自动更新】功能，只需要关闭插件`开放 API`并刷新白板即可。
...
- 按`Alt`+ `L`将内容块依次转为图片用的`dataUrl`
  ...
  - 这个处理会有比较慢，完成后，笔记软件右上角会弹出提示：`已完成`
...
- 将笔记中的文档/内容块拖拽到白板中生成卡片后，首次拖拽卡片前需要先单击卡片的边缘区域或者白板空白处（这样能让白板重新获得焦点）。
- 如果在已有其他内容的文档中误嵌入该挂件，可参考：[误操作插入挂件后如何撤销?](https://github.com/BryceAndJuly/Whiteboard/issues/48)
...
对于当前版本，打开挂件文件夹`Whiteboard`——`assets`——`index-ZsssFvwm.js`,在该js文件中搜索：
